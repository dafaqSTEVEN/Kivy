...
with open("E:\\Program\\GITHUBKIVY\\MY\\Kivy\\bus_en\\routes.csv") as q:
...
for i in range(len(route_short_name)):
    if route_short_name[i] not in route_dict:
        route_dict[str(route_short_name[i])] = [route_id[i], agency_id[i], route_short_name[i], route_long_name[i],route_type[i], route_url[i]]
    else:
        for g in range (15):
            if route_short_name[i] in route_dict and str(route_short_name[i]) + ('_' + str(g+1)) not in route_dict:
                route_dict[str(route_short_name[i]+'_'+str(g))] = [route_id[i], agency_id[i], route_short_name[i], route_long_name[i],route_type[i], route_url[i]]
